...
class InstantXControlNetFluxFormat2(Flux):
    def __init__(
        ...
        **kwargs,
        ...
        kwargs.pop("depth_single_blocks_controlnet", None)
        if "out_channels" not in kwargs:
            kwargs["out_channels"] = 320
        ...
        self.controlnet_x_embedder = zero_module(
            operations.Linear(self.in_channels, self.hidden_size).to(
    ...
    def forward_orig(
        ...
        if self.union:
            if controlnet_mode is None:
                raise ValueError(
                    "using union-controlnet, but controlnet_mode is not a list or is empty"
                )
            controlnet_mode = torch.tensor(controlnet_mode).to(
                self.device, dtype=torch.long
            )
            controlnet_mode = controlnet_mode.reshape([-1, 1])
            emb_controlnet_mode = self.controlnet_mode_embedder(controlnet_mode).to(
                self.dtype
            )
        ...
        for block in self.single_transformer_blocks:
            ...
            single_block_res_samples = single_block_res_samples + (
                img[:, txt.shape[1] :],
            )
        ...
        for single_block_res_sample, single_controlnet_block in zip(
            single_block_res_samples, self.controlnet_single_blocks
        ):
            single_block_res_sample = single_controlnet_block(single_block_res_sample)
            controlnet_single_block_res_samples = (
                controlnet_single_block_res_samples + (single_block_res_sample,)
            )
        ...
        interval_control_double = int(
            np.ceil(n_double_blocks / len(controlnet_block_res_samples))
        )
        for i in range(n_double_blocks):
            index = i // interval_control_double
            expanded_controlnet_block_res_samples.append(
                controlnet_block_res_samples[index]
            )

        # Expand controlnet_single_block_res_samples to match n_single_blocks
        expanded_controlnet_single_block_res_samples = []
        interval_control_single = int(
            np.ceil(n_single_blocks / len(controlnet_single_block_res_samples))
        )
        for i in range(n_single_blocks):
            index = i // interval_control_single
            expanded_controlnet_single_block_res_samples.append(
                controlnet_single_block_res_samples[index]
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        return self.forward_orig(
            img, img_ids, hint, context, txt_ids, timesteps, y, guidance, control_type
        )
